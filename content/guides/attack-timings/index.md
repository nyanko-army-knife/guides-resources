...
![attack cycle image (if it doesn't display properly here, try referring to the one in dkaf's original document.)](../static/images/attack%20cycle.png)

What is **Attack Cooldown?** It’s the time between the actual attack and the start of the next attack
...
`Foreswing + attack cooldown = attack rate (121f + 479f = 600f)`.
...
In other words, Maglev’s backswing is longer than its attack cooldown, meaning that the latter does not
